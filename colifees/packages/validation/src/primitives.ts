...
/**
 * A COLIFEES serial. Accepts the label's spacing and casing, normalises to the
...
export const serialNumber = z
  ...
  .pipe(z.string().regex(/^CLF\d{8,12}$/, 'must be a COLIFEES serial number, for example CLF26000001'));

/** Opaque QR token. Length-bounded and character-restricted before any lookup. */
export const qrToken = z
  .string()
  .trim()
  .regex(/^[A-Za-z0-9_-]{22,64}$/, 'not a valid COLIFEES QR code');

export const claimNumber = z
  .string()
  .trim()
  .toUpperCase()
  .regex(/^CLM\d{8,12}$/, 'must be a COLIFEES claim number');
