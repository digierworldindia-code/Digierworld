<?php
declare(strict_types=1);

$pageTitle       = 'Mithaas | Indian Sweets, Bakery & Restaurant';
$metaDescription = 'Mithaas is a sweet shop, a daily bakery and a full vegetarian restaurant under one roof — traditional mithai, fresh cakes and breads, North Indian, South Indian, chaat and Chinese.';

require __DIR__ . '/includes/header.php';
?>


<main id="main">

  <!-- ============================ HERO ============================ -->
  <section class="hero" aria-label="Mithaas — Sweets, Bakery and Restaurant">
    <div class="hero__media" aria-hidden="true">
      <img src="assets/img/hero/hero-sweets.jpg" alt=""
           data-photo data-label="Mithaas" loading="eager" fetchpriority="high" decoding="async">
    </div>
    <div class="hero__scrim" aria-hidden="true"></div>

    <div class="container">
      <div class="hero__inner">
        <img class="hero__logo" data-logo="hero" src="assets/img/brand/mithaas-logo.png"
             alt="Mithaas — Sweets, Bakery, Restaurant" width="660" height="410"
             fetchpriority="high" decoding="async">
        <h1 class="hero__title">Where Tradition <em>Meets Taste</em></h1>
        <p class="hero__sub">
          Beautifully crafted sweets, freshly baked favourites and flavours made for every occasion.
        </p>
        <div class="btn-row btn-row--center hero__actions">
          <a class="btn btn-gold" href="menu.php">Explore Our Menu</a>
          <a class="btn btn-on-dark" href="contact.php">Visit Mithaas</a>
        </div>
      </div>
    </div>
    <p class="hero__scroll" aria-hidden="true">Scroll</p>
  </section>

  <!-- ======================= BRAND STATEMENT ======================= -->
  <section class="section section--tight">
    <div class="container container--narrow text-center">
      <p class="eyebrow eyebrow--center" data-reveal>Sweets &middot; Bakery &middot; Restaurant</p>
      <p class="display-3" data-reveal style="max-width:22ch;margin-inline:auto">
        Three counters. One address. Whatever the occasion asks for.
      </p>
      <div data-reveal><svg class="ornament" viewBox="0 0 160 26" fill="none" aria-hidden="true" focusable="false"><path d="M4 13h52M104 13h52" stroke="currentColor" stroke-width="1"/><path d="M80 3c5.4 5.2 8.1 8.5 8.1 10S85.4 20.8 80 23c-5.4-2.2-8.1-8.5-8.1-10S74.6 8.2 80 3z" stroke="currentColor" stroke-width="1"/><path d="M80 8.5c2 2.2 3 3.6 3 4.5s-1 2.3-3 4.5c-2-2.2-3-3.6-3-4.5s1-2.3 3-4.5z" fill="currentColor" opacity=".55"/><circle cx="62" cy="13" r="2" fill="currentColor"/><circle cx="98" cy="13" r="2" fill="currentColor"/><circle cx="56" cy="13" r="1" fill="currentColor" opacity=".6"/><circle cx="104" cy="13" r="1" fill="currentColor" opacity=".6"/></svg></div>
    </div>
  </section>

  <!-- ========================= BRAND STORY ========================= -->
  <section class="section section--cream" aria-labelledby="story-h">
    <div class="container">
      <div class="row g-5 align-items-center">
        <div class="col-lg-6 order-lg-2">
          <div class="editorial-media" data-reveal="curtain">
            <div class="fig ar-45 fig--zoom"><img src="assets/img/brand/story-counter.jpg" alt="Sweets being arranged at the Mithaas counter" data-photo data-label="Sweets being arranged at the Mithaas counter" loading="lazy" decoding="async"></div>
          </div>
        </div>
        <div class="col-lg-6">
          <div data-reveal>
            <p class="eyebrow">Our Story</p>
            <h2 class="display-2 mb-4" id="story-h">A Little Sweetness.<br>A Lot of Memories.</h2>
            <p class="lead dropcap">
              Nobody remembers the box. They remember who brought it, and what the house
              smelled like when it was opened.
            </p>
            <p>
              That is really the whole business. A card arrives, a result comes through, someone
              gets married, someone comes home after a long time away — and before anyone says
              anything sensible, somebody is sent out for mithai. We have been on the other end of
              that errand for a while now, and we take it seriously.
            </p>
            <p>
              So the kadhai goes on early. Chhena is set, khoya is stirred down, the oven is warm by
              the time the shutters go up. Through the day the counter is refilled rather than
              stocked once, because a laddoo made at four in the afternoon is a different thing from
              one made at six in the morning. In the kitchen behind it, dal sits on a low flame,
              dosa batter finishes fermenting, and lunch begins.
            </p>
            <p class="pullquote">
              We would rather make less of something and make it properly.
            </p>
            <p class="mt-4 mb-0">
              <a class="tlink" href="our-story.php">Read our story <i class="bi bi-arrow-right" aria-hidden="true"></i></a>
            </p>
          </div>
        </div>
      </div>
    </div>
  </section>

  <!-- ======================== THE FAVOURITES ======================== -->
  <section class="section" aria-labelledby="fav-h">
    <div class="container">
      <div class="sechead row g-4 align-items-end">
        <div class="col-lg-8" data-reveal>
          <p class="eyebrow">Signature</p>
          <h2 class="display-2" id="fav-h">The Mithaas Favourites</h2>
        </div>
        <div class="col-lg-4 text-lg-end" data-reveal>
          <a class="tlink" href="sweets.php">All our sweets <i class="bi bi-arrow-right" aria-hidden="true"></i></a>
        </div>
      </div>

      <div class="shelf" data-stagger>
        <article class="sweet" data-reveal>
          <div class="fig ar-45 sweet__fig fig--zoom"><img src="assets/img/sweets/gulab-jamun.jpg" alt="Gulab Jamun" data-photo data-label="Gulab Jamun" loading="lazy" decoding="async"></div>
          <p class="sweet__idx">01</p>
          <h3 class="sweet__name">Gulab Jamun</h3>
          <p class="sweet__desc">Soft, warm, and soaked right through with cardamom syrup.</p>
          <p class="sweet__meta"><span>250g / 500g / 1kg</span>
            <a class="tlink" href="menu.php#traditional-mithai" aria-label="See Gulab Jamun on the menu">
              Details <i class="bi bi-arrow-right" aria-hidden="true"></i></a>
          </p>
        </article>
        <article class="sweet" data-reveal>
          <div class="fig ar-45 sweet__fig fig--zoom"><img src="assets/img/sweets/kaju-katli.jpg" alt="Kaju Katli" data-photo data-label="Kaju Katli" loading="lazy" decoding="async"></div>
          <p class="sweet__idx">02</p>
          <h3 class="sweet__name">Kaju Katli</h3>
          <p class="sweet__desc">Thin diamonds of cashew and sugar, finished with silver leaf.</p>
          <p class="sweet__meta"><span>250g / 500g / 1kg</span>
            <a class="tlink" href="menu.php#traditional-mithai" aria-label="See Kaju Katli on the menu">
              Details <i class="bi bi-arrow-right" aria-hidden="true"></i></a>
          </p>
        </article>
        <article class="sweet" data-reveal>
          <div class="fig ar-45 sweet__fig fig--zoom"><img src="assets/img/sweets/motichoor-laddoo.jpg" alt="Motichoor Laddoo" data-photo data-label="Motichoor Laddoo" loading="lazy" decoding="async"></div>
          <p class="sweet__idx">03</p>
          <h3 class="sweet__name">Motichoor Laddoo</h3>
          <p class="sweet__desc">Tiny pearls of boondi pressed into a laddoo that gives way at first bite.</p>
          <p class="sweet__meta"><span>250g / 500g / 1kg</span>
            <a class="tlink" href="menu.php#traditional-mithai" aria-label="See Motichoor Laddoo on the menu">
              Details <i class="bi bi-arrow-right" aria-hidden="true"></i></a>
          </p>
        </article>
        <article class="sweet" data-reveal>
          <div class="fig ar-45 sweet__fig fig--zoom"><img src="assets/img/sweets/rasmalai.jpg" alt="Rasmalai" data-photo data-label="Rasmalai" loading="lazy" decoding="async"></div>
          <p class="sweet__idx">04</p>
          <h3 class="sweet__name">Rasmalai</h3>
          <p class="sweet__desc">Chhena discs resting in thickened, cardamom-scented milk.</p>
          <p class="sweet__meta"><span>4 pc / 8 pc</span>
            <a class="tlink" href="menu.php#traditional-mithai" aria-label="See Rasmalai on the menu">
              Details <i class="bi bi-arrow-right" aria-hidden="true"></i></a>
          </p>
        </article>
        <article class="sweet" data-reveal>
          <div class="fig ar-45 sweet__fig fig--zoom"><img src="assets/img/sweets/kaju-anjeer-roll.jpg" alt="Kaju Anjeer Roll" data-photo data-label="Kaju Anjeer Roll" loading="lazy" decoding="async"></div>
          <p class="sweet__idx">05</p>
          <h3 class="sweet__name">Kaju Anjeer Roll</h3>
          <p class="sweet__desc">Cashew wrapped around a fig centre, sliced thin.</p>
          <p class="sweet__meta"><span>250g / 500g / 1kg</span>
            <a class="tlink" href="menu.php#traditional-mithai" aria-label="See Kaju Anjeer Roll on the menu">
              Details <i class="bi bi-arrow-right" aria-hidden="true"></i></a>
          </p>
        </article>
        <article class="sweet" data-reveal>
          <div class="fig ar-45 sweet__fig fig--zoom"><img src="assets/img/sweets/ghewar.jpg" alt="Ghewar" data-photo data-label="Ghewar" loading="lazy" decoding="async"></div>
          <p class="sweet__idx">06</p>
          <h3 class="sweet__name">Ghewar</h3>
          <p class="sweet__desc">The monsoon disc of lace-thin batter, soaked and topped with malai.</p>
          <p class="sweet__meta"><span>In season</span>
            <a class="tlink" href="menu.php#traditional-mithai" aria-label="See Ghewar on the menu">
              Details <i class="bi bi-arrow-right" aria-hidden="true"></i></a>
          </p>
        </article></div>
    </div>
  </section>

  <!-- ===================== SWEETS EXPERIENCE ===================== -->
  <section class="section section--cream" aria-labelledby="sweets-h">
    <div class="container">
      <div class="sechead text-center">
        <p class="eyebrow eyebrow--center" data-reveal>The Sweet Counter</p>
        <h2 class="display-2" id="sweets-h" data-reveal>Four Ways To Say Congratulations</h2>
        <p class="lead mt-3" data-reveal style="margin-inline:auto">
          Everything is sold by weight, packed while you wait, and boxed properly if it is going somewhere.
        </p>
      </div>
      
      <div class="row g-5 align-items-center mb-5 pb-lg-4">
        <div class="col-lg-6">
          <div data-reveal>
            <p class="panel__num">01</p>
            <h3 class="display-3 mb-3">Traditional Mithai</h3>
            <p class="lead">Gulab jamun, laddoo, burfi, jalebi — the sweets that turn up at every festival, every exam result and every ordinary Tuesday that needed improving.</p>
            <ul class="panel__list"><li>Laddoo</li><li>Burfi</li><li>Jalebi</li><li>Peda</li><li>Rasgulla</li><li>Halwa</li></ul>
            <p class="mt-4 mb-0"><a class="tlink" href="menu.php#traditional-mithai">
              See the traditional mithai <i class="bi bi-arrow-right" aria-hidden="true"></i></a></p>
          </div>
        </div>
        <div class="col-lg-6">
          <div data-reveal="curtain">
            <div class="fig ar-43 fig--zoom"><img src="assets/img/sweets/cat-traditional-mithai.jpg" alt="Traditional Mithai" data-photo data-label="Traditional Mithai" loading="lazy" decoding="async"></div>
          </div>
        </div>
      </div>
      <div class="row g-5 align-items-center flex-lg-row-reverse mb-5 pb-lg-4">
        <div class="col-lg-6">
          <div data-reveal>
            <p class="panel__num">02</p>
            <h3 class="display-3 mb-3">Premium Sweets</h3>
            <p class="lead">More dry fruit, more patience, and a box that looks like it means something. These are the ones that go out for weddings and Diwali.</p>
            <ul class="panel__list"><li>Kaju Katli</li><li>Anjeer</li><li>Pista Roll</li><li>Badam Barfi</li><li>Gift Boxes</li></ul>
            <p class="mt-4 mb-0"><a class="tlink" href="menu.php#premium-sweets">
              See the premium sweets <i class="bi bi-arrow-right" aria-hidden="true"></i></a></p>
          </div>
        </div>
        <div class="col-lg-6">
          <div data-reveal="curtain">
            <div class="fig ar-43 fig--zoom"><img src="assets/img/sweets/cat-premium-sweets.jpg" alt="Premium Sweets" data-photo data-label="Premium Sweets" loading="lazy" decoding="async"></div>
          </div>
        </div>
      </div>
      <div class="row g-5 align-items-center mb-5 pb-lg-4">
        <div class="col-lg-6">
          <div data-reveal>
            <p class="panel__num">03</p>
            <h3 class="display-3 mb-3">Regional Favourites</h3>
            <p class="lead">Ghewar from Rajasthan, petha from Agra, mysore pak from the south. India makes sweets differently in every direction, and we've learned a few of them.</p>
            <ul class="panel__list"><li>Ghewar</li><li>Petha</li><li>Mysore Pak</li><li>Sandesh</li></ul>
            <p class="mt-4 mb-0"><a class="tlink" href="menu.php#regional-seasonal">
              See the regional favourites <i class="bi bi-arrow-right" aria-hidden="true"></i></a></p>
          </div>
        </div>
        <div class="col-lg-6">
          <div data-reveal="curtain">
            <div class="fig ar-43 fig--zoom"><img src="assets/img/sweets/cat-regional-seasonal.jpg" alt="Regional Favourites" data-photo data-label="Regional Favourites" loading="lazy" decoding="async"></div>
          </div>
        </div>
      </div>
      <div class="row g-5 align-items-center flex-lg-row-reverse mb-5 pb-lg-4">
        <div class="col-lg-6">
          <div data-reveal>
            <p class="panel__num">04</p>
            <h3 class="display-3 mb-3">Seasonal Specials</h3>
            <p class="lead">Gajar halwa when the carrots are red, ghewar through the monsoon, thandai at Holi. Some things are worth waiting for.</p>
            <ul class="panel__list"><li>Gajar Halwa</li><li>Moong Dal Halwa</li><li>Malpua</li><li>Thandai</li></ul>
            <p class="mt-4 mb-0"><a class="tlink" href="menu.php#regional-seasonal">
              See the seasonal specials <i class="bi bi-arrow-right" aria-hidden="true"></i></a></p>
          </div>
        </div>
        <div class="col-lg-6">
          <div data-reveal="curtain">
            <div class="fig ar-43 fig--zoom"><img src="assets/img/sweets/cat-regional-seasonal.jpg" alt="Seasonal Specials" data-photo data-label="Seasonal Specials" loading="lazy" decoding="async"></div>
          </div>
        </div>
      </div>
      <div class="text-center">
        <a class="btn btn-ghost" href="sweets.php" data-reveal>Explore The Sweets</a>
      </div>
    </div>
  </section>

  <!-- =========================== BAKERY =========================== -->
  <section class="section" aria-labelledby="bakery-h">
    <div class="container">
      <div class="sechead row g-4 align-items-end">
        <div class="col-lg-7" data-reveal>
          <p class="eyebrow">The Bakery</p>
          <h2 class="display-2" id="bakery-h">Fresh From The Oven</h2>
        </div>
        <div class="col-lg-5" data-reveal>
          <p class="lead mb-0">
            The oven starts before the shop does. Bread and cookies come out first, cakes
            through the morning, and by evening there is usually not much left to argue over.
          </p>
        </div>
      </div>

      <div class="row g-4" data-stagger>
        <div class="col-sm-6 col-lg-4" data-reveal>
          <a class="tile fig ar-43 fig--zoom d-block" href="menu.php#bakery">
            <img src="assets/img/bakery/truffle-cake.jpg" alt="Cakes at Mithaas"
                 data-photo data-label="Cakes" loading="lazy" decoding="async">
            <span class="tile__cap">
              <h3>Cakes</h3>
              <p>Truffle, butterscotch, fresh fruit — and photo cakes with a day's notice.</p>
            </span>
          </a>
        </div>
        <div class="col-sm-6 col-lg-4" data-reveal>
          <a class="tile fig ar-43 fig--zoom d-block" href="menu.php#bakery">
            <img src="assets/img/bakery/pastry-counter.jpg" alt="Pastries at Mithaas"
                 data-photo data-label="Pastries" loading="lazy" decoding="async">
            <span class="tile__cap">
              <h3>Pastries</h3>
              <p>Cut fresh from the day's cakes, kept cold until you ask.</p>
            </span>
          </a>
        </div>
        <div class="col-sm-6 col-lg-4" data-reveal>
          <a class="tile fig ar-43 fig--zoom d-block" href="menu.php#bakery">
            <img src="assets/img/bakery/cookies.jpg" alt="Cookies &amp; Rusk at Mithaas"
                 data-photo data-label="Cookies &amp; Rusk" loading="lazy" decoding="async">
            <span class="tile__cap">
              <h3>Cookies &amp; Rusk</h3>
              <p>Ajwain, jeera, coconut, chocolate chip. Sold by weight.</p>
            </span>
          </a>
        </div>
        <div class="col-sm-6 col-lg-4" data-reveal>
          <a class="tile fig ar-43 fig--zoom d-block" href="menu.php#bakery">
            <img src="assets/img/bakery/bread.jpg" alt="Breads &amp; Buns at Mithaas"
                 data-photo data-label="Breads &amp; Buns" loading="lazy" decoding="async">
            <span class="tile__cap">
              <h3>Breads &amp; Buns</h3>
              <p>Loaves, pav and burger buns, baked in batches through the day.</p>
            </span>
          </a>
        </div>
        <div class="col-sm-6 col-lg-4" data-reveal>
          <a class="tile fig ar-43 fig--zoom d-block" href="menu.php#bakery">
            <img src="assets/img/bakery/veg-puff.jpg" alt="Baked Snacks at Mithaas"
                 data-photo data-label="Baked Snacks" loading="lazy" decoding="async">
            <span class="tile__cap">
              <h3>Baked Snacks</h3>
              <p>Veg puff, paneer puff, pizza puff — the four o'clock regulars.</p>
            </span>
          </a>
        </div>
        <div class="col-sm-6 col-lg-4" data-reveal>
          <a class="tile fig ar-43 fig--zoom d-block" href="menu.php#bakery">
            <img src="assets/img/bakery/brownie.jpg" alt="Doughnuts &amp; Brownies at Mithaas"
                 data-photo data-label="Doughnuts &amp; Brownies" loading="lazy" decoding="async">
            <span class="tile__cap">
              <h3>Doughnuts &amp; Brownies</h3>
              <p>For whoever is coming home from school.</p>
            </span>
          </a>
        </div></div>

      <div class="text-center mt-5 pt-3">
        <a class="btn btn-ghost" href="bakery.php" data-reveal>Visit The Bakery</a>
      </div>
    </div>
  </section>

  <!-- ========================= RESTAURANT ========================= -->
  <section class="section section--espresso" aria-labelledby="rest-h">
    <div class="container">
      <div class="row g-5 align-items-center mb-5">
        <div class="col-lg-6">
          <div data-reveal="curtain">
            <div class="fig ar-43 fig--zoom" data-plate-dark><img src="assets/img/restaurant/thali-hero.jpg" alt="A full vegetarian thali served at Mithaas" data-photo data-label="A full vegetarian thali served at Mithaas" loading="lazy" decoding="async"></div>
          </div>
        </div>
        <div class="col-lg-6">
          <div data-reveal>
            <p class="eyebrow">The Restaurant</p>
            <h2 class="display-2 mb-4" id="rest-h">More Than Mithai.<br>A Table Full of Flavour.</h2>
            <p class="lead">
              Past the sweet counter there is a proper kitchen, and it is busier than most people
              expect. Breakfast is dosa and idli. Lunch is thali. By evening it is chaat at the
              front, noodles in the wok, and somebody's birthday cake coming out of the fridge.
            </p>
            <p class="mt-4 mb-0">
              <a class="tlink" href="restaurant.php">See the restaurant <i class="bi bi-arrow-right" aria-hidden="true"></i></a>
            </p>
          </div>
        </div>
      </div>

      <div class="row g-4" data-stagger>
        <div class="col-md-6 col-xl-4" data-reveal>
          <div class="value h-100">
            <h3>North Indian</h3>
            <p>Paneer and dal cooked to order, breads off the tandoor, rice, raita and a full thali.</p>
            <ul class="panel__list"><li>Paneer</li><li>Dal Makhani</li><li>Tandoori Roti</li><li>Biryani</li><li>Thali</li></ul>
            <p class="mt-3 mb-0"><a class="tlink" href="menu.php#north-indian">
              View <i class="bi bi-arrow-right" aria-hidden="true"></i></a></p>
          </div>
        </div>
        <div class="col-md-6 col-xl-4" data-reveal>
          <div class="value h-100">
            <h3>South Indian</h3>
            <p>Batter ground in-house and left overnight, which is the whole difference.</p>
            <ul class="panel__list"><li>Masala Dosa</li><li>Idli</li><li>Medu Vada</li><li>Uttapam</li><li>Filter Coffee</li></ul>
            <p class="mt-3 mb-0"><a class="tlink" href="menu.php#south-indian">
              View <i class="bi bi-arrow-right" aria-hidden="true"></i></a></p>
          </div>
        </div>
        <div class="col-md-6 col-xl-4" data-reveal>
          <div class="value h-100">
            <h3>Snacks &amp; Street Food</h3>
            <p>The chaat counter at the front, assembled while you watch.</p>
            <ul class="panel__list"><li>Golgappa</li><li>Samosa Chaat</li><li>Pav Bhaji</li><li>Chole Bhature</li><li>Rolls</li></ul>
            <p class="mt-3 mb-0"><a class="tlink" href="menu.php#snacks-street-food">
              View <i class="bi bi-arrow-right" aria-hidden="true"></i></a></p>
          </div>
        </div>
        <div class="col-md-6 col-xl-4" data-reveal>
          <div class="value h-100">
            <h3>Chinese</h3>
            <p>Cooked hot and fast in a wok. Dry or gravy, your call.</p>
            <ul class="panel__list"><li>Manchurian</li><li>Chilli Paneer</li><li>Hakka Noodles</li><li>Momos</li><li>Fried Rice</li></ul>
            <p class="mt-3 mb-0"><a class="tlink" href="menu.php#chinese">
              View <i class="bi bi-arrow-right" aria-hidden="true"></i></a></p>
          </div>
        </div>
        <div class="col-md-6 col-xl-4" data-reveal>
          <div class="value h-100">
            <h3>Beverages</h3>
            <p>Chai boiled properly, lassi in a tall glass, shakes for the afternoon.</p>
            <ul class="panel__list"><li>Masala Chai</li><li>Filter Coffee</li><li>Lassi</li><li>Shakes</li><li>Mojito</li></ul>
            <p class="mt-3 mb-0"><a class="tlink" href="menu.php#beverages">
              View <i class="bi bi-arrow-right" aria-hidden="true"></i></a></p>
          </div>
        </div>
        <div class="col-md-6 col-xl-4" data-reveal>
          <div class="value h-100">
            <h3>Desserts</h3>
            <p>Falooda, kulfi, and hot gulab jamun with cold vanilla.</p>
            <ul class="panel__list"><li>Falooda</li><li>Kulfi</li><li>Rabri Jalebi</li><li>Ice Cream</li></ul>
            <p class="mt-3 mb-0"><a class="tlink" href="menu.php#desserts">
              View <i class="bi bi-arrow-right" aria-hidden="true"></i></a></p>
          </div>
        </div></div>
    </div>
  </section>

  <!-- ========================= MENU TEASER ========================= -->
  <section class="section section--tight section--sand" aria-labelledby="menu-h">
    <div class="container text-center">
      <p class="eyebrow eyebrow--center" data-reveal>The Menu</p>
      <h2 class="display-2" id="menu-h" data-reveal>Everything, In One Place</h2>
      <p class="lead mt-3" data-reveal style="margin-inline:auto">
        Eleven sections, searchable, and built to be read on a phone while you are deciding.
      </p>
      <div class="btn-row btn-row--center mt-4" data-reveal>
        <a class="btn" href="menu.php">Browse The Full Menu</a>
      </div>
      <div data-reveal><svg class="ornament" viewBox="0 0 160 26" fill="none" aria-hidden="true" focusable="false"><path d="M4 13h52M104 13h52" stroke="currentColor" stroke-width="1"/><path d="M80 3c5.4 5.2 8.1 8.5 8.1 10S85.4 20.8 80 23c-5.4-2.2-8.1-8.5-8.1-10S74.6 8.2 80 3z" stroke="currentColor" stroke-width="1"/><path d="M80 8.5c2 2.2 3 3.6 3 4.5s-1 2.3-3 4.5c-2-2.2-3-3.6-3-4.5s1-2.3 3-4.5z" fill="currentColor" opacity=".55"/><circle cx="62" cy="13" r="2" fill="currentColor"/><circle cx="98" cy="13" r="2" fill="currentColor"/><circle cx="56" cy="13" r="1" fill="currentColor" opacity=".6"/><circle cx="104" cy="13" r="1" fill="currentColor" opacity=".6"/></svg></div>
    </div>
  </section>

  <!-- ======================== CELEBRATIONS ======================== -->
  <section class="section section--wine" aria-labelledby="celeb-h">
    <div class="container">
      <div class="row g-5 align-items-center">
        <div class="col-lg-6">
          <div data-reveal>
            <p class="eyebrow">Gifting &amp; Celebrations</p>
            <h2 class="display-2 mb-4" id="celeb-h">Made For Celebrations</h2>
            <p class="lead">
              Diwali boxes that go out by the dozen. A tray for the office on somebody's last day.
              Five kilos of laddoo for a wedding, packed the night before so it is ready when
              the cars leave.
            </p>
            <p>
              Tell us the occasion, roughly how many people, and what you would like to spend.
              We will put something together and you can taste it before you commit to it.
            </p>
            <div class="btn-row mt-4">
              <a class="btn btn-on-dark" data-action="whatsapp"
                 data-message="Hello Mithaas, I'd like to ask about gifting and celebration orders." href="#">Explore Gifting</a>
              <a class="btn btn-on-dark" href="contact.php">Talk To Us</a>
            </div>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="row g-3" data-stagger>
            <div class="col-7" data-reveal><div class="fig ar-34 fig--zoom" data-plate-dark><img src="assets/img/sweets/gift-box.jpg" alt="A packed Mithaas sweet box" data-photo data-label="A packed Mithaas sweet box" loading="lazy" decoding="async"></div></div>
            <div class="col-5" data-reveal><div class="fig ar-45 fig--zoom" data-plate-dark><img src="assets/img/sweets/festival-tray.jpg" alt="A festival tray of assorted sweets" data-photo data-label="A festival tray of assorted sweets" loading="lazy" decoding="async"></div></div>
            <div class="col-12" data-reveal><div class="fig ar-21 fig--zoom" data-plate-dark><img src="assets/img/sweets/wedding-order.jpg" alt="Sweets packed for a wedding order" data-photo data-label="Sweets packed for a wedding order" loading="lazy" decoding="async"></div></div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <!-- ========================= WHY MITHAAS ========================= -->
  <section class="section" aria-labelledby="why-h">
    <div class="container">
      <div class="sechead text-center">
        <p class="eyebrow eyebrow--center" data-reveal>Why Mithaas</p>
        <h2 class="display-2" id="why-h" data-reveal>What We Pay Attention To</h2>
      </div>
      <div class="values" data-stagger>
      <div class="value" data-reveal>
        <p class="value__n">01</p>
        <h3>Freshness</h3>
        <p>Sweets are made in small batches through the day, not once in the morning. What you take home was almost certainly made a few hours ago.</p>
      </div>
      <div class="value" data-reveal>
        <p class="value__n">02</p>
        <h3>Craftsmanship</h3>
        <p>Dal makhani sits on low heat overnight. Dosa batter is ground and left to ferment. Some things can't be hurried, so we don't.</p>
      </div>
      <div class="value" data-reveal>
        <p class="value__n">03</p>
        <h3>Variety</h3>
        <p>A box of mithai, a birthday cake, and dinner for four — without driving to three different places.</p>
      </div>
      <div class="value" data-reveal>
        <p class="value__n">04</p>
        <h3>Hospitality</h3>
        <p>The counter staff know most faces and remember most orders. That's not a policy, it's just how it happens.</p>
      </div></div>
    </div>
  </section>

  <!-- =========================== GALLERY =========================== -->
  <section class="section section--cream" aria-labelledby="gal-h">
    <div class="container">
      <div class="sechead row g-4 align-items-end">
        <div class="col-lg-8" data-reveal>
          <p class="eyebrow">Gallery</p>
          <h2 class="display-2" id="gal-h">A Look Around</h2>
        </div>
        <div class="col-lg-4 text-lg-end" data-reveal>
          <a class="tlink" href="gallery.php">Open the gallery <i class="bi bi-arrow-right" aria-hidden="true"></i></a>
        </div>
      </div>
      <div class="row g-3 g-lg-4" data-stagger>
        <div class="col-6 col-lg-3" data-reveal>
          <a href="gallery.php" aria-label="The sweet counter at Mithaas — open the gallery">
            <div class="fig ar-34 fig--zoom"><img src="assets/img/gallery/sweet-counter.jpg" alt="The sweet counter at Mithaas" data-photo data-label="The sweet counter at Mithaas" loading="lazy" decoding="async"></div>
          </a>
        </div>
        <div class="col-6 col-lg-3" data-reveal>
          <a href="gallery.php" aria-label="A full vegetarian thali — open the gallery">
            <div class="fig ar-1 fig--zoom"><img src="assets/img/gallery/thali.jpg" alt="A full vegetarian thali" data-photo data-label="A full vegetarian thali" loading="lazy" decoding="async"></div>
          </a>
        </div>
        <div class="col-6 col-lg-3" data-reveal>
          <a href="gallery.php" aria-label="Fresh bakery shelf — open the gallery">
            <div class="fig ar-34 fig--zoom"><img src="assets/img/gallery/bakery-shelf.jpg" alt="Fresh bakery shelf" data-photo data-label="Fresh bakery shelf" loading="lazy" decoding="async"></div>
          </a>
        </div>
        <div class="col-6 col-lg-3" data-reveal>
          <a href="gallery.php" aria-label="The dining room at Mithaas — open the gallery">
            <div class="fig ar-1 fig--zoom"><img src="assets/img/gallery/dining.jpg" alt="The dining room at Mithaas" data-photo data-label="The dining room at Mithaas" loading="lazy" decoding="async"></div>
          </a>
        </div></div>
    </div>
  </section>

  <!-- ========================= TESTIMONIALS ========================= -->
  <section class="section" aria-labelledby="rev-h">
    <div class="container">
      <div class="sechead text-center">
        <p class="eyebrow eyebrow--center" data-reveal>In Their Words</p>
        <h2 class="display-2" id="rev-h" data-reveal>What People Tell Us</h2>
      </div>
      <div class="row g-4" data-testimonials data-stagger></div>
      <p class="text-center menu-count mt-4" data-reveal>
        Reviews are published here exactly as customers write them.
      </p>
    </div>
  </section>

  <!-- ========================== INSTAGRAM ========================== -->
  <section class="section section--tight section--cream" aria-labelledby="ig-h">
    <div class="container">
      <div class="sechead text-center">
        <p class="eyebrow eyebrow--center" data-reveal>
          <span data-field="instagramHandle" data-pending="Instagram">Instagram</span>
        </p>
        <h2 class="display-3" id="ig-h" data-reveal>Fresh Off The Counter</h2>
      </div>
      <div class="ig-grid" data-stagger>
        <a data-action="instagram" href="#" aria-label="See more on the Mithaas Instagram">
          <div class="fig ar-1 fig--zoom"><img src="assets/img/gallery/ig-01.jpg" alt="Mithaas on Instagram — photo 1" data-photo data-label="Mithaas on Instagram — photo 1" loading="lazy" decoding="async"></div>
        </a>
        <a data-action="instagram" href="#" aria-label="See more on the Mithaas Instagram">
          <div class="fig ar-1 fig--zoom"><img src="assets/img/gallery/ig-02.jpg" alt="Mithaas on Instagram — photo 2" data-photo data-label="Mithaas on Instagram — photo 2" loading="lazy" decoding="async"></div>
        </a>
        <a data-action="instagram" href="#" aria-label="See more on the Mithaas Instagram">
          <div class="fig ar-1 fig--zoom"><img src="assets/img/gallery/ig-03.jpg" alt="Mithaas on Instagram — photo 3" data-photo data-label="Mithaas on Instagram — photo 3" loading="lazy" decoding="async"></div>
        </a>
        <a data-action="instagram" href="#" aria-label="See more on the Mithaas Instagram">
          <div class="fig ar-1 fig--zoom"><img src="assets/img/gallery/ig-04.jpg" alt="Mithaas on Instagram — photo 4" data-photo data-label="Mithaas on Instagram — photo 4" loading="lazy" decoding="async"></div>
        </a>
        <a data-action="instagram" href="#" aria-label="See more on the Mithaas Instagram">
          <div class="fig ar-1 fig--zoom"><img src="assets/img/gallery/ig-05.jpg" alt="Mithaas on Instagram — photo 5" data-photo data-label="Mithaas on Instagram — photo 5" loading="lazy" decoding="async"></div>
        </a>
        <a data-action="instagram" href="#" aria-label="See more on the Mithaas Instagram">
          <div class="fig ar-1 fig--zoom"><img src="assets/img/gallery/ig-06.jpg" alt="Mithaas on Instagram — photo 6" data-photo data-label="Mithaas on Instagram — photo 6" loading="lazy" decoding="async"></div>
        </a></div>
      <div class="text-center mt-4">
        <a class="tlink" data-action="instagram" href="#">
          Follow Mithaas <i class="bi bi-arrow-right" aria-hidden="true"></i></a>
      </div>
    </div>
  </section>

  <!-- =========================== LOCATION =========================== -->
  <section class="section section--espresso" aria-labelledby="loc-h" data-schema-localbusiness>
    <div class="container">
      <div class="row g-5">
        <div class="col-lg-5">
          <div data-reveal>
            <p class="eyebrow">Find Us</p>
            <h2 class="display-2 mb-4" id="loc-h">Come And See Us</h2>

            <div class="infoitem">
              <span class="infoitem__k">Address</span>
              <span class="infoitem__v" data-field="address" data-pending="Address to be confirmed"
                    style="white-space:pre-line"></span>
            </div>
            <div class="infoitem">
              <span class="infoitem__k">Landmark</span>
              <span class="infoitem__v" data-field="landmark" data-pending="Landmark to be confirmed"></span>
            </div>
            <div class="infoitem">
              <span class="infoitem__k">Phone</span>
              <span class="infoitem__v"><a data-action="call" href="#"
                    ><span data-field="phone" data-pending="To be confirmed"></span></a></span>
            </div>

            <h3 class="h4 mt-5 mb-3">Opening Hours</h3>
            <ul class="hours" data-hours></ul>

            <div class="btn-row mt-4">
              <a class="btn btn-on-dark" data-action="directions" href="#">Get Directions</a>
              <a class="btn btn-on-dark" data-action="whatsapp" href="#">WhatsApp</a>
            </div>
          </div>
        </div>
        <div class="col-lg-7">
          <div class="mapframe" data-map data-reveal="fade"></div>
        </div>
      </div>
    </div>
  </section>

</main>

<?php require __DIR__ . '/includes/footer.php'; ?>
