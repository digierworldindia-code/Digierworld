 ...
 */
import { withStaffContext, type PrismaClient } from '@polyfix/database';
...
export class AuthService {
  ...
  async login(
    ...
    if (user.dealerLink && (user.dealerLink.dealer.status !== 'ACTIVE' || user.dealerLink.dealer.deletedAt)) {
      ...
      throw errors.forbidden(
        'dealer not active',
        `This dealership account is not active. Please contact ${BRAND.shortName}.`,
      );
    }
  ...
  async changePassword(
    ...
    const policy = checkPasswordPolicy(newPassword, [user.email, user.fullName, 'polyfix']);
  ...
  async resetPassword(token: string, newPassword: string, meta: RequestMeta): Promise<void> {
    ...
    const policy = checkPasswordPolicy(newPassword, [record.user.email, record.user.fullName, 'polyfix']);
    if (!policy.ok) throw errors.validation({ newPassword: policy.problems });
