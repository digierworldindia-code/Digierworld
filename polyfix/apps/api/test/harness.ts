...
import { getPrisma, type PrismaClient } from '@polyfix/database';
import { hashPassword } from '@polyfix/auth';
...
export async function seedWorld(suffix: string): Promise<TestWorld> {
  ...
  return staffScope(async (tx) => {
    ...
    const admin = await makeUser(`admin.${suffix}@test.polyfix.local`, 'Test Admin', 'ADMIN');
    const warehouse = await makeUser(`wh.${suffix}@test.polyfix.local`, 'Test Warehouse', 'WAREHOUSE');
    const warrantyManager = await makeUser(`wm.${suffix}@test.polyfix.local`, 'Test Warranty Manager', 'WARRANTY_MANAGER');
    ...
    const makeDealer = async (index: number) => {
      ...
      const user = await tx.user.create({
        data: {
          email: `dealer${index}.${suffix}@test.polyfix.local`,
...
export function multipartBody(
  ...
  const boundary = `----polyfixtest${Date.now()}${Math.random().toString(36).slice(2)}`;
  const parts: Buffer[] = [];
