...
const envSchema = z.object({
  ...
  // Overridden per deployment; the default is derived from the brand so a
  // rename never leaves a stale company name on outgoing mail.
  MAIL_FROM: z.string().default(`${BRAND.name} <no-reply@${BRAND.web.defaultSiteUrl.replace(/^https?:\/\//, '')}>`),
