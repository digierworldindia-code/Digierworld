...
/**
 * Reads one generated identifier from a database sequence function.
 *
 * The function name is restricted to a known set rather than interpolated
 * freely — this is the one place in the seed that builds SQL from a string.
 */
async function nextIdentifier(
  tx: Prisma.TransactionClient,
  fn: 'colifees_next_serial' | 'colifees_next_batch_code' | 'colifees_next_dealer_code',
): Promise<string> {
  const rows = await tx.$queryRawUnsafe<{ value: string }[]>(`SELECT ${fn}() AS value`);
  const value = rows[0]?.value;
  if (!value) throw new Error(`${fn} returned no value`);
  return value;
}
...
async function seedSuperAdmin(tx: Prisma.TransactionClient): Promise<void> {
  // Default only, for a fresh install. An existing administrator's address is
  // never rewritten by the seed: an email is a login credential, not branding.
  const defaultAdminEmail = `admin@${BRAND.web.defaultSiteUrl.replace(/^https?:\/\//, '')}`;
  const email = (process.env.SEED_ADMIN_EMAIL ?? defaultAdminEmail).toLowerCase().trim();
  ...
  if (supplied) {
    const check = checkPasswordPolicy(supplied, [email, 'polyfix']);
    if (!check.ok) {
  ...
  const user = await tx.user.create({
    data: {
      email,
      fullName: process.env.SEED_ADMIN_NAME ?? `${BRAND.shortName} Super Admin`,
...
async function seedDemo(tx: Prisma.TransactionClient): Promise<void> {
  ...
  const warehouse = await tx.warehouse.upsert({
    ...
    create: {
      code: 'WH-JAIPUR',
      name: `${BRAND.shortName} Jaipur Plant`,
  ...
  for (const spec of dealerSpecs) {
    ...
    const dealer = found
      ...
      : await tx.dealer.create({
          data: {
            code: await nextIdentifier(tx, 'colifees_next_dealer_code'),
  ...
  if (existingBatch) {
    ...
    const batchCode = await nextIdentifier(tx, 'colifees_next_batch_code');
    ...
    for (let i = 0; i < 40; i += 1) {
      ...
      const serial = await nextIdentifier(tx, 'colifees_next_serial');
